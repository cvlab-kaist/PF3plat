...
@dataclass
class DatasetRE10k_TESTCfg(DatasetCfgCommon):
    name: Literal["re10k_test"]
    roots: list[Path]
...
class DatasetRE10kTest(IterableDataset):
    cfg: DatasetRE10k_TESTCfg
    stage: Stage
    ...
    def __iter__(self):
        ...
        for i, scene_path in enumerate(self.all_scenes):
            ...
            example["context"]["extrinsics"]=example["context"]["extrinsics"][0]
            example["context"]["intrinsics"]=example["context"]["intrinsics"][0]
            ...
            example["target"]["extrinsics"]=example["target"]["extrinsics"][0]
            example["target"]["intrinsics"]=example["target"]["intrinsics"][0]
            example["target"]["image"]=example["target"]["image"][0]
