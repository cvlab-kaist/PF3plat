...
from ..unidepthv2.decoder import Decoder
...
class UniDepthV2(
    ...
    def forward(self, inputs, image_metas, num_views):
        if inputs["image"].ndim == 3:
            inputs["image"] = inputs["image"].unsqueeze(0)
        if inputs["K"] is not None and inputs["K"].ndim == 2:
        ...
        if inputs["image"].max() > 5 or inputs["image"].dtype == torch.uint8:
            inputs["image"] = inputs["image"].to(torch.float32).div(255)
        if inputs["image"].min() >= 0.0 and inputs["image"].max() <= 1.0:
            inputs["image"] = TF.normalize(
                inputs["image"],
        ...
        inputs["image"], inputs["K"] = _preprocess(
            inputs["image"],
            inputs["K"],
            (h, w),
            ratio,
    ...
    def infer(self, rgbs: torch.Tensor, intrinsics=None):
        ...
        rays, angles = generate_rays(intrinsics, (h, w))
    ...
    def build(self, config):
        mod = importlib.import_module("src.model.unidepth.encoder")
