...
def spherical_to_euclidean(spherical_tensor) :
    theta = spherical_tensor[..., 0]  # Extract polar angle
    phi = spherical_tensor[..., 1]  # Extract azimuthal angle
...
def euclidean_to_spherical(spherical_tensor) :
...
def euclidean_to_spherical_zbuffer(euclidean_tensor) :
...
def downsample(data, downsample_factor = 2):
    N, _, H, W = data.shape
    data = data.view(
        N,
        H // downsample_factor,
